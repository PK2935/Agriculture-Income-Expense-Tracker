...
const IncomeForm = () => {
  ...
  const handleAddEntry = () => {
    setEntries([
      ...entries,
      { id: uuidv4(), title: '', source: '', amount: 0, date: new Date().toISOString().split('T')[0] },
    ]);
  };
  ...
  const handleRemoveEntry = async (index) => {
    ...
    if (userId && entryToRemove.id) {
      try {
        ...
        if (userDoc.exists()) {
          const userData = userDoc.data();
          const entryIndex = userData.income.entries.findIndex((entry) => entry.id === entryToRemove.id);
          if (entryIndex !== -1) {
            const updatedEntries = [...userData.income.entries];
            updatedEntries.splice(entryIndex, 1);
            await updateDoc(userRef, {
              'income.entries': updatedEntries,
              'income.totalIncome': userData.income.totalIncome - Number(entryToRemove.amount),
            });
          }
        }
